...
class SrsLayout {
  ...
  addCenteredTitle(
    ...
    const width = maxWidth ?? (PAGE_WIDTH - MARGIN * 2)
    const lines = wrapText(text, width, size, bold)
    const leading = Math.round(size * 1.35)
    let currentY = y
    for (const line of lines) {
      this.drawText(line, PAGE_WIDTH / 2, currentY, { size, font: bold ? 'bold' : 'regular', align: 'center', color })
      currentY -= leading
    }
...
function buildCoverPage(data: ArchitectureData): PageContent {
  ...
  let currentY = PAGE_HEIGHT - 94
  
  // 1. Software Requirements Specification header
  currentY -= page.addCenteredTitle('SOFTWARE REQUIREMENTS SPECIFICATION', currentY, 16, true, [0.35, 0.35, 0.38])
  currentY -= 20
  
  // 2. Project Name
  currentY -= page.addCenteredTitle(data.projectName, currentY, 28, true, [0.08, 0.08, 0.09])
  currentY -= 24
  
  // 3. Project Summary (Description)
  currentY -= page.addCenteredTitle(ascii(data.summary), currentY, 12, false, [0.22, 0.22, 0.24])
  currentY -= 16
  
  // 4. Industry Label
  currentY -= page.addCenteredTitle(data.domainProfile.industryLabel, currentY, 11, false, [0.38, 0.38, 0.42])
  currentY -= 40
  
  // 5. Metadata box
  const boxHeight = 120
  const rectY = currentY - 100 // bottom of rect
  page.drawRect(MARGIN, rectY, PAGE_WIDTH - MARGIN * 2, boxHeight, true, true, [0.98, 0.98, 0.99])
  
  const textOffsetY = rectY + boxHeight - 40
  page.drawText('Version', MARGIN + 18, textOffsetY, { size: 10, font: 'bold', color: [0.2, 0.2, 0.22] })
  page.drawText('v1.0', MARGIN + 110, textOffsetY, { size: 10, color: [0.12, 0.12, 0.13] })
  
  page.drawText('Generation Date', MARGIN + 18, textOffsetY - 20, { size: 10, font: 'bold', color: [0.2, 0.2, 0.22] })
  page.drawText(data.metadata.generatedAt, MARGIN + 110, textOffsetY - 20, { size: 10, color: [0.12, 0.12, 0.13] })
  
  page.drawText('Generated By', MARGIN + 18, textOffsetY - 40, { size: 10, font: 'bold', color: [0.2, 0.2, 0.22] })
  page.drawText('FoundryForge', MARGIN + 110, textOffsetY - 40, { size: 10, color: [0.12, 0.12, 0.13] })
  
  page.drawText('Document Type', MARGIN + 18, textOffsetY - 60, { size: 10, font: 'bold', color: [0.2, 0.2, 0.22] })
  page.drawText('Software Requirements Specification', MARGIN + 110, textOffsetY - 60, { size: 10, color: [0.12, 0.12, 0.13] })
  
  // 6. Footer note (at the bottom of page)
